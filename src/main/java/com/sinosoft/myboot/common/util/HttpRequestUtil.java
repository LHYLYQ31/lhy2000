...
public class HttpRequestUtil {
    ...
    private static String sendPostReq(String url, String param, Boolean isEncode) {
        ...
        try {
            ...
            conn.setReadTimeout(200000);//设置读取超时时间          
            conn.setConnectTimeout(200000);//设置连接超时时间 
            // 发送POST请求必须设置如下两行
    ...
    /**
     * 
     * <B>方法名称：jsonToForm</B><BR>
     * <B>概要说明：将json转成&字符串</B><BR>
     *
     * @author：lihaiyi
     * @cretetime:2018年5月21日 下午6:00:58
     * @param param
     * @return String
     */
    public static String jsonToForm(net.sf.json.JSONObject param) {
        List<String> params = new ArrayList<>();
        for (@SuppressWarnings("unchecked")
        Iterator<String> iter = param.keys(); iter.hasNext();) {
            String key = iter.next();
            String value = param.getString(key);
            if (value.startsWith("[")) {
                String[] values = getStringArray(value);
                for (String string : values) {
                    String paramString = key + "[]=" + string;
    ...
    /**
     * <B>方法名称：getStrings</B><BR>
     * <B>概要说明：</B><BR>
     *
     * @return
     */
    private static String[] getStringArray(String result) {
